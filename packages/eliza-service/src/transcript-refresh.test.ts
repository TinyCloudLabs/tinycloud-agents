...
const raw="Synthetic transcript";
const snapshot=JSON.stringify({contractVersion:3,meetingRef:"meeting",source:"fireflies",sourceId:"source",operationId:"op",createdAt:"2030-01-01T00:00:00Z",metadata:{title:"Fixture",startedAt:null,organizerEmail:null,participants:[],metadata:{}},body:{basis:"transcript",encoding:"utf-8",schema:"text",raw,original:{digest:sha256(raw),byteLength:Buffer.byteLength(raw),recordCount:1,extent:"unknown",captureComplete:null},omissions:[]},overview:null,aliases:[]});
const request={contractVersion:3,reference:{meetingRef:"meeting",source:"fireflies",sourceId:"source",revision:sha256(snapshot)},basis:"transcript"} as const;
        ...
        async useDelegation(delegation) {
          ...
          return {
            ...
            kv: { get: async (key: string, options?: { prefix?: string }) => {
              ...
              return result("kv", { data: snapshot });
            } },
          };
        },
      };
...
describe("transcript activated-session renewal", () => {
  test("reads SQL and KV after child expiry using the same live parent and a fresh node", async () => {
    ...
    advance(61 * MINUTE);
    expect((await reader.getMetadata!("meeting"))?.meetingRef).toBe("meeting");
    expect(await reader.readEvidence(request)).toMatchObject({ state: "complete" });
  ...
  test("concurrent SQL and KV calls share one refresh", async () => {
    ...
    const results = Promise.allSettled([reader.getMetadata!("meeting"), reader.readEvidence(request)]);
  ...
  for (const cancellation of ["abort", "deadline"] as const) test(`${cancellation} stops one refresh waiter without blocking another caller`, async () => {
    ...
    const other = reader.readEvidence(request).then(value => ({ value }), error => ({ error }));
