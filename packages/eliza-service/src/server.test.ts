...
describe("eliza-service HTTP server", () => {
  ...
  it("GET /capabilities authenticates and reports an immutable configured build revision", async () => {
    ...
    try {
      ...
      expect(await response.json()).toEqual({ meetingRetrieval: { contractVersion: 2 }, buildRevision: process.env.BUILD_REVISION, chatTasks: { version: 1, enabled: false, cancellation: true, providerProfile: "tinychat-redpill", models: [] } });
